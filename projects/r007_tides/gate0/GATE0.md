...
Proposed as **r007**, 2026-09-10. Backlog id `I58` (§2 Maps and real geography → accent
...
**This is the concept's known weakness and it is not being hidden.** `r006` failed on exactly this
...
| The payoff frame shows an object the viewer has never seen | **No.** The Sun, the Moon, the Earth and its ocean. First draft failed this — the Earth was a bare grey disc, which is the `I51` "legible but nameable by nobody" trap — and was redrawn with a real ocean and landmasses. |
...
the public domain. No `r006`-style rebuild is waiting. The physics needs no licence at all.
...
- **Motion is the top risk, as always.** Two panels and a caption is the shape that scores badly
  ...
  `r006`'s 16% route-draw beat. Target ≥50% event density at `--width 240`.
...
The question is about the **sentence**, not the picture — the I15 lesson, which cost a full build:
...
Yes → build as r007. No → `I58` is parked with the reason recorded; do not repair it, pick a
different id.
