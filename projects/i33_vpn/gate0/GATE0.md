...
Proposed as **r009**. `I33` is a narrowing of the backlog row "What a VPN hides, and four
...
3. **One repeatable thing** — *it moves your address, not your clock.* Carried into the
   argument without the reel, which is the property r007 never had.
...
1. **The top risk is motion, not truth.** Rule 4 is measured on large-area change: r007's
...
3. **Licence.** tzdb is public domain. GSHHG was cleared for r006. Nothing else is needed and
   no third-party dataset is involved, so there is no r007-style rebuild waiting.
...
Yes → build as r009. No → `I33` is parked with the reason recorded; do not repair it.
