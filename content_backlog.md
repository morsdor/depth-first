...
| ID | Hook (on-screen title) | They already know | They have never seen | On screen | Family |
|:--|:--|:--|:--|:--|:--|
| **I15** ★ | One route across Manhattan. Two ways to find it. | Every route your phone has given you | Dijkstra expands in all directions. A\* adds one term — straight-line distance still to go — and the search collapses into a beam at the destination | Split screen on one real city graph, both frontiers expanding, node counters diverging violently | MapRoute + Counter |
*(Hook corrected 2026-09-10. "Dijkstra checks 300,000 roads. A* checks 300" was a figure from memory and a real graph does not support it: on 13,797 junctions of Midtown Manhattan, Times Square to the Met, Dijkstra expands **7,597** and A* expands **2,099** — **3.6x**, not 1000x. Across four Midtown routes the ratio ran 2.8x-4.2x; central London as a control gave 3.1x and 6.6x. Measured by `projects/i15_astar/search.py` on a graph pulled from OpenStreetMap. The grid-weakens-the-heuristic explanation is suggestive, not established — see the gate note.)*
