...
*65 vertical post concepts for the Depth First short-form line. Written 2026-09-02 alongside
...
| **I51** | The obvious way to shuffle is wrong  (retired — 2026-09-10, built as r005 and never posted) | Every shuffle button you have pressed | `for i: swap(a[i], a[random 0..n-1])` has n^n equally likely runs over n! orders. For n=3 that is 27/6 = 4.5 — not a whole number, so some order MUST be likelier. Fisher–Yates shrinks the range and gives exactly n! runs, one per order | Two landing-position maps side by side from a real 400,000-run trial — where every card starts (row) vs where it ends (column). The naive map grows a bright staircase and a dark wedge; Fisher–Yates stays featureless. Deviation from fair: 8.34% vs 0.46% | Heatmap + Counter |
...
*(Added 2026-09-07 as r005. NOT a rename of `I07` — that entry is about clumping in a CORRECT shuffle and remains unbuilt. Ids are permanent, so a different concept gets a different id.)*
...
*Built as `r005` (five rebuilds, v5 landing-position map, 39 s) and **never posted**. Project deleted 2026-09-10 on the decision not to publish it; recoverable from git history. Reel number r005 is spent. The lessons — a legible shape is not a recognisable object, the payoff frame must not argue against its own caption, event density measures change not information — are in `brand_guide_software.md` §13 and are among the most cited in the repo.*
...
*(FAILED Gate 0 on the sentence, 2026-09-10 — proposed as r009 and rejected before any build. Three sentences were offered and none was one a viewer would repeat: the rebuilt-world framing has no dispute running behind it, and the two that do — text seeds reaching 1 world in 4.3 billion (`String.hashCode` is 32-bit against a 64-bit seed), and the pre-Beta-1.8 Far Lands at 12,550,824 blocks where noise x 171.103 overflows a 32-bit int — are arguments only Minecraft players are having. **The fight has to be one a non-specialist is already in**; r006 landed inside flat earth, and a seed-sharing dispute is not that. Not repaired, per `CLAUDE.md`. Also corrected while here: "layered noise turns coordinates into height" is the PRE-1.18 generator; since 1.18 the overworld is 3D density functions and splines. Licence was clear — own noise code in brand colours, no Mojang assets, and [Cubiomes](https://github.com/Cubitect/cubiomes) is MIT if real seeds are ever wanted.)*

*Built as `r008` (28 s, real OpenStreetMap Paris graph) and **never posted** — it passed Gate 0 on its picture and the finished reel drew "I didn't understand the point. We are comparing 2 algos?". Project deleted 2026-09-10; recoverable from git history. Reel number r008 is spent. **Measured figures, corrected from this row's original claim:** on 26,193 junctions of central Paris, Dijkstra expands 17,092 and A\* expands 1,700 — **10.1x, not the 1000x the hook promised**. Manhattan gave only 3.6x.*

| **I53** | Your street is the bottom of a bowl you can't see — ❌ **FAILED Gate 0, 2026-09-10** | Every monsoon, the same junctions under water | *(claim falsified — see below)* | Real SRTM terrain, water routing downhill, the junctions the news names | MapRoute + Counter |
*(Added and killed the same day — the first concept under the widened "how systems work" remit, and the first killed by measurement rather than judgement. Proposed sentence: "your street floods because it's the bottom of a bowl, and a satellite names the same three junctions every year." Tested on real SRTM 1-arcsec over Bengaluru (1260x1080 cells, georeference verified against published elevations) against eight junctions the city reports flooded by name, controlled on 3,000 random points in the built-up core. **All three metrics went the wrong way**: flood spots sit at the 57.4th elevation percentile of their own 1 km neighbourhood vs 47.7 for random points, have a 20.5 m rim above them vs 24.0, and LESS flow accumulation (1.23 vs 1.34 log10). Only Silk Board is genuinely in a dip. 30 m SRTM cannot see an underpass and better data is not free, so this does not improve by retrying. NOT repaired and no city shopping — testing Mumbai and Chennai until one fits is p-hacking when the causal claim is the reel. Full record in `projects/i53_flood/gate0/GATE0.md`.)*
| **I56** | Your flight home is an hour shorter | The outbound and return times never match | A river of air over 250 km/h sits at cruise altitude. The aircraft rides it one way and fights it the other, so the two routes are not even the same line | Real wind field, both tracks bending around the jet, a clock running on each | MapRoute + Counter |
| **I57** | Your shower costs more in electricity than in water | Everyone pays a water bill | Bengaluru's supply is pumped roughly 100 km and lifted about 500 m. m·g·h puts that near 1.4 Wh per litre before a drop is treated or delivered | Real terrain profile from the river to the city, the lift drawn to scale, watt-hours accumulating per litre | MapRoute + Counter |
| **I58** | Two high tides a day, and one of them shouldn't be there — ✅ **BUILT as r009, 2026-09-10** *(re-spined: see note)* | Everyone has seen a tide come in | The moon pulls the near side hardest. The FAR bulge is the one almost every explanation gets wrong, and "centrifugal force" is not the answer | Real harmonic constituents for one named port, both bulges, a month of predicted tide against the measured record | Diagram + Counter |
*(`I58` built 2026-09-10 as r009, and the row's own framing was corrected during Gate 0 rather than
shipped. **"Two high tides a day" is not universally true** — tides are semidiurnal, diurnal or mixed
by location, and the Gulf of Mexico gets one high a day. **The two bulges are also not equal**: the
far one is 4.9% weaker. And the equilibrium two-bulge picture is the FORCING, not the observed water
level — real tides are set by basin resonance and amphidromic systems, so a reel animating bulges
sweeping around the Earth would teach the textbook simplification as fact. r009 is therefore about
the tide-raising force: the Sun pulls 178.7x harder than the Moon and raises 0.459x the tide.
Measured, not recalled — `projects/r009_tides/tides.py`.)*

| **I59** | Thirty thousand things are over your head right now | You have watched a satellite cross the sky | Public orbital elements and real propagation put every tracked object on a map for tonight — and show how much of low orbit now belongs to one operator | Real TLEs propagated, the passes over one city in the next 90 minutes | MapRoute + Counter |
| **I60** | North is running to Siberia | Every compass and every map | The magnetic pole has moved further in living memory than in the whole nineteenth century, and fast enough that the world magnetic model had to be re-issued off-schedule | Real declination field, the pole's measured track by decade, the angle your compass is wrong by where you live | MapRoute + Counter |
| **I61** | There is a 660-tonne ball hanging inside that tower | Taipei 101's damper is a tourist attraction with a gift shop | It is a pendulum tuned to the building's own period, so it swings LATE and pulls against the sway instead of with it | Two towers in the same quake, one damped, sway amplitude plotted under both | Diagram + Counter |
| **I62** | That building is standing on rubber | Everyone has felt or feared a quake | Base isolation lengthens the building's period past the ground's, so the shaking arrives and the building declines to join in | The same recorded ground motion, two buildings, drift measured at the top of each | Diagram + Counter |
...
| **I64** | The queue you switched out of speeds up  (shelved — 2026-09-10, built and stopped at Gate 3; concept still open) | Everyone has changed lanes and lost | You were never in "the slow queue" — there is one fast queue and it is not yours. One shared line at identical staffing cuts everyone's wait several-fold, and the shops that know this already did it | Two shops side by side, same arrivals and same tills: separate lanes backing up while the single snake stays short | Diagram + Counter |
| **I65** | A traffic jam with no cause | You have crawled for ten minutes and then found nothing there | Cars driving a ring at a steady speed still produce a jam, and the jam travels BACKWARDS through the traffic. It has been done with real cars on a real track | Real car-following simulation, the jam condensing out of nothing and moving upstream against the flow | Diagram + Counter |
| **I66** | The lights aren't badly timed — you're in the wrong platoon | Everyone believes their city's signals are broken | A green wave is timed for one speed. Drive faster than it and you will meet every red; the road is not punishing you, it is metering you | Real road geometry and offsets, two cars at different speeds, one sailing through and one stopping at each | MapRoute + Counter |
| **I67** | Your green tariff doesn't change the electrons | Everyone has been sold a renewable plan | The wall hands you whatever was running three seconds ago. The tariff is an accounting instrument, and the grid publishes what actually ran | Live grid mix and carbon intensity hour by hour, against what the plan claims | Counter + Diagram |

*Built and **shelved at Gate 3** ("the output is not sound") despite passing every automated gate at 65% event density — the best in the repo at the time. Project deleted 2026-09-10; recoverable from git history. It never held a reel number. **Still open**: the Gate 0 sentence passed a human gate and nobody has diagnosed which part of the execution failed, so a rebuild starts there, not at a re-render.*
