...
| **I51** | The obvious way to shuffle is wrong | Every shuffle button you have pressed | `for i: swap(a[i], a[random 0..n-1])` has n^n equally likely runs over n! orders. For n=3 that is 27/6 = 4.5 — not a whole number, so some order MUST be likelier. Fisher–Yates shrinks the range and gives exactly n! runs, one per order | Six bars, one per order, filling from a real 100,000-run trial: lumpy, then flat | Chart + Counter |
